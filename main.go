...
var defaultCommands = []CommandSpec{
	...
	// weka_events_leader_too_slow omitted here — generated by buildWindowedDefaultCmds with the collection time window.
...
// buildWindowedDefaultCmds returns the small set of default commands whose --start-time
// must match the collection window rather than a fixed relative offset.
...
func buildWindowedDefaultCmds(from, to time.Time) []CommandSpec {
	...
	startFlag := fmt.Sprintf(" --start-time %s", effectiveFrom.Format("2006-01-02T15:04:05"))
	return []CommandSpec{

...
func filterByTimeWindow(paths []string, from, to time.Time) ([]string, int) {
	...
	for _, k := range order {
		...
		for i, f := range files {
			...
			// Non-rotated (current active) log files are always collected regardless
			// of mtime. A file like upgrade.log or syslog.log may not have been
			// written to recently (e.g. no upgrade in the last 8 hours) but still
			// contains relevant state. Time filtering only applies to rotated files
			// (.log.1, .log.2, …) where mtime brackets tell us what content period
			// each file covers.
			if !f.rotated {
				result = append(result, f.path)
				continue
...
var defaultCommands = []CommandSpec{
	...
	{Name: "weka_cluster_container", Cmd: "weka cluster container"},
	...
	{Name: "weka_local_ps", Cmd: "weka local ps -v", NodeLocal: true, LocalOnly: true},
	...
	// weka_events_major is generated by buildWindowedDefaultCmds with the collection
	// time window. Do NOT add it here — it needs --start-time matching the window.
	...
	{Name: "weka_debug_snapviews", Cmd: "weka debug config show snapViews"},
	...
	// These use intentionally short fixed windows: they answer "is anything wrong RIGHT NOW?"
	// not "what happened during the collection window". No --per-process — cluster-average
	// is sufficient for anomaly detection and avoids overloading large clusters.
	...
	{Name: "weka_stats_outliers", Cmd: "weka stats --show-internal --stat PUMPS_TXQ_FULL,BAD_RECV_CSUM,CORRUPT_PACKETS,RDMA_COMP_STATUSES --interval 600"},
	{Name: "weka_stats_obs_errors", Cmd: "weka stats --category object_storage --stat RESPONSE_COUNT_BAD_GATEWAY,RESPONSE_COUNT_GATEWAY_TIMEOUT,RESPONSE_COUNT_HTTP_VERSION_NOT_SUPPORTED,RESPONSE_COUNT_NOT_IMPLEMENTED,RESPONSE_COUNT_SERVER_ERROR,RESPONSE_COUNT_SERVICE_UNAVAILABLE --start-time -30m -Z"},
}

// buildWindowedDefaultCmds returns default commands whose --start-time must align with
// the collection window. Uses a relative offset (e.g. -9h) rather than an absolute
// timestamp — weka events only reliably accepts relative offsets for --start-time.
// Contrast with the diagnostic stats in defaultCommands that intentionally use short
// fixed windows (e.g. -10m for RDMA errors) to capture current cluster state.
func buildWindowedDefaultCmds(from, to time.Time) []CommandSpec {
	effectiveFrom := from
	if from.IsZero() {
		effectiveFrom = time.Now().Add(-8 * time.Hour)
	}
	// Convert absolute time to a relative offset that weka events accepts.
	// Round up by one hour so we don't miss events right at the window boundary.
	hours := int(time.Since(effectiveFrom).Hours()) + 1
	startFlag := fmt.Sprintf(" --start-time -%dh", hours)
	return []CommandSpec{
		// All events at major+ severity within the collection window.
		{Name: "weka_events_major", Cmd: "weka events --severity major" + startFlag},
	}
}
...
func buildPerfCommands(from, to time.Time) []CommandSpec {
	...
	if mode == "per-process" {
		...
		return []CommandSpec{
			...
			stats("weka_stats_ssd_read_reqs", " --show-internal --stat SSD_READ_REQS --per-process"),
			stats("weka_stats_ssd_writes", " --show-internal --stat SSD_WRITES --per-process"),
			stats("weka_stats_ssd_blocks_read", " --show-internal --stat SSD_BLOCKS_READ --per-process"),
			stats("weka_stats_ssd_blocks_written", " --show-internal --stat SSD_BLOCKS_WRITTEN --per-process"),
			stats("weka_stats_drive_read_ops", " --show-internal --stat DRIVE_READ_OPS --per-process"),
			stats("weka_stats_drive_write_ops", " --show-internal --stat DRIVE_WRITE_OPS --per-process"),
			stats("weka_stats_drive_load", " --show-internal --stat DRIVE_LOAD --per-process"),
			stats("weka_stats_drive_active_ios", " --show-internal --stat DRIVE_ACTIVE_IOS --per-process"),
			// Filesystem ops (client-facing)
			stats("weka_stats_ops_reads", " --show-internal --category ops --stat READS --per-process"),
			stats("weka_stats_ops_writes", " --show-internal --category ops --stat WRITES --per-process"),
			stats("weka_stats_read_latency", " --show-internal --category ops --stat READ_LATENCY --per-process"),
			stats("weka_stats_write_latency", " --show-internal --category ops --stat WRITE_LATENCY --per-process"),
			stats("weka_stats_ops_read_bytes", " --show-internal --category ops --stat READ_BYTES --per-process"),
			stats("weka_stats_ops_write_bytes", " --show-internal --category ops --stat WRITE_BYTES --per-process"),
			stats("weka_stats_ops_count", " --show-internal --category ops --stat OPS --per-process"),
			stats("weka_stats_ops_throughput", " --show-internal --category ops --stat THROUGHPUT --per-process"),
			// Driver ops (kernel/DPDK layer)
			stats("weka_stats_ops_driver_reads", " --show-internal --category ops_driver --stat READS --per-process"),
			stats("weka_stats_ops_driver_writes", " --show-internal --category ops_driver --stat WRITES --per-process"),
			stats("weka_stats_ops_driver_read_bytes", " --show-internal --category ops_driver --stat READ_BYTES --per-process"),
			stats("weka_stats_ops_driver_write_bytes", " --show-internal --category ops_driver --stat WRITE_BYTES --per-process"),
			stats("weka_stats_ops_driver_throughput", " --show-internal --category ops_driver --stat THROUGHPUT --per-process"),
			// Network
			...
			// JRPC
			stats("weka_stats_jrpc_processing_avg", " --show-internal --stat JRPC_SERVER_PROCESSING_AVG --per-process"),
			stats("weka_stats_jrpc_processing_time", " --show-internal --stat JRPC_SERVER_PROCESSING_TIME --per-process"),
			stats("weka_stats_jrpc_calls", " --show-internal --stat JRPC_SERVER_CALLS_CLIENT_SUPPORTS_QOS --per-process"),
			// RPC
			stats("weka_stats_rpc_client_calls", " --show-internal --stat CLIENT_RPC_CALLS --per-process"),
			stats("weka_stats_rpc_roundtrip_avg", " --show-internal --stat CLIENT_ROUNDTRIP_AVG --per-process"),
			stats("weka_stats_rpc_server_calls", " --show-internal --stat SERVER_RPC_CALLS --per-process"),
			stats("weka_stats_rpc_server_processing_avg", " --show-internal --stat SERVER_PROCESSING_AVG --per-process"),
			stats("weka_stats_rpc_timeouts", " --show-internal --stat CLIENT_RECEIVED_TIMEOUTS --per-process"),
...
var nfsCommands = []CommandSpec{
	...
	{Name: "weka_local_resources_ganesha", Cmd: "weka local resources -C ganesha", Profile: ProfileNFS, NodeLocal: true},
...
var logFileSpecs = []LogFileSpec{
	...
	//
	// Directory structure (confirmed from live clusters):
	//   /opt/weka/logs/quota.json                        top-level quota snapshot
	//   /opt/weka/logs/<container>/syslog.log*           depth-1 syslog
	//   /opt/weka/logs/<container>/upgrade.log*          depth-1 upgrade log
	//   /opt/weka/logs/<container>/upgrade_local.log     depth-1 upgrade local log
	//   /opt/weka/logs/<container>/upgrade_report_*.json depth-1 upgrade reports
	//   /opt/weka/logs/<container>/traces.txt            depth-1 trace summary
	//   /opt/weka/logs/<container>/weka/output.log*      depth-2 main weka process
	//   /opt/weka/logs/<container>/weka/shelld.log*      depth-2 shell daemon
	//   /opt/weka/logs/<container>/weka/atop.stats*      depth-2 atop perf stats
	//   /opt/weka/logs/<container>/weka/events.log       depth-2 weka events
	//   /opt/weka/logs/<container>/wtracer/              depth-2 wtracer dumps
	//   /opt/weka/logs/<container>/nginx/                depth-2 nginx access/error
	//   /opt/weka/logs/<container>/tmp/atop.stats*       depth-2 atop in tmp/
	//   Containers: drives*, compute*, frontend*, client*, dataserv*, default*
	//
	// Two depth levels cover everything. seenSrcPaths deduplicates overlaps.
	//
	// Core containers are always collected; protocol containers (ganesha*, s3*,
	// envoy*, smbw*) only when the matching profile is active.

	// ── top-level files ───────────────────────────────────────────────────
	{SrcGlob: "/opt/weka/logs/quota.json", DestDir: "weka/containers"},

	// ── core containers (always collected) ───────────────────────────────
	// Covers drives*, compute*, frontend*, client*, dataserv*, default*.
	...
	{SrcGlob: "/opt/weka/logs/drives*/*.txt", DestDir: "weka/containers"},
	{SrcGlob: "/opt/weka/logs/drives*/*/*.log*", DestDir: "weka/containers"},
	{SrcGlob: "/opt/weka/logs/drives*/*/*.stats*", DestDir: "weka/containers"},
	{SrcGlob: "/opt/weka/logs/compute*/*.log*", DestDir: "weka/containers"},
	{SrcGlob: "/opt/weka/logs/compute*/*.json", DestDir: "weka/containers"},
	{SrcGlob: "/opt/weka/logs/compute*/*.txt", DestDir: "weka/containers"},
	{SrcGlob: "/opt/weka/logs/compute*/*/*.log*", DestDir: "weka/containers"},
	{SrcGlob: "/opt/weka/logs/compute*/*/*.stats*", DestDir: "weka/containers"},
	{SrcGlob: "/opt/weka/logs/frontend*/*.log*", DestDir: "weka/containers"},
	{SrcGlob: "/opt/weka/logs/frontend*/*.json", DestDir: "weka/containers"},
	{SrcGlob: "/opt/weka/logs/frontend*/*.txt", DestDir: "weka/containers"},
	{SrcGlob: "/opt/weka/logs/frontend*/*/*.log*", DestDir: "weka/containers"},
	{SrcGlob: "/opt/weka/logs/frontend*/*/*.stats*", DestDir: "weka/containers"},
	{SrcGlob: "/opt/weka/logs/client*/*.log*", DestDir: "weka/containers"},
	{SrcGlob: "/opt/weka/logs/client*/*.json", DestDir: "weka/containers"},
	{SrcGlob: "/opt/weka/logs/client*/*.txt", DestDir: "weka/containers"},
	{SrcGlob: "/opt/weka/logs/client*/*/*.log*", DestDir: "weka/containers"},
	{SrcGlob: "/opt/weka/logs/client*/*/*.stats*", DestDir: "weka/containers"},
	// dataserv* — data server container (present on some cluster configs)
	{SrcGlob: "/opt/weka/logs/dataserv*/*.log*", DestDir: "weka/containers"},
	{SrcGlob: "/opt/weka/logs/dataserv*/*.json", DestDir: "weka/containers"},
	{SrcGlob: "/opt/weka/logs/dataserv*/*.txt", DestDir: "weka/containers"},
	{SrcGlob: "/opt/weka/logs/dataserv*/*/*.log*", DestDir: "weka/containers"},
	{SrcGlob: "/opt/weka/logs/dataserv*/*/*.stats*", DestDir: "weka/containers"},
	// default — legacy/fallback container name
	{SrcGlob: "/opt/weka/logs/default*/*.log*", DestDir: "weka/containers"},
	{SrcGlob: "/opt/weka/logs/default*/*.json", DestDir: "weka/containers"},
	{SrcGlob: "/opt/weka/logs/default*/*.txt", DestDir: "weka/containers"},
	{SrcGlob: "/opt/weka/logs/default*/*/*.log*", DestDir: "weka/containers"},
	{SrcGlob: "/opt/weka/logs/default*/*/*.stats*", DestDir: "weka/containers"},
	...
	{SrcGlob: "/opt/weka/logs/ganesha*/*.conf", DestDir: "weka/containers", Profile: ProfileNFS},
	{SrcGlob: "/opt/weka/logs/ganesha*/*.out", DestDir: "weka/containers", Profile: ProfileNFS},
	{SrcGlob: "/opt/weka/logs/ganesha*/*.stats*", DestDir: "weka/containers", Profile: ProfileNFS},
	{SrcGlob: "/opt/weka/logs/ganesha*/*/*.log*", DestDir: "weka/containers", Profile: ProfileNFS},
	...
	{SrcGlob: "/opt/weka/logs/s3*/*.stats*", DestDir: "weka/containers", Profile: ProfileS3},
	{SrcGlob: "/opt/weka/logs/s3*/*/*.log*", DestDir: "weka/containers", Profile: ProfileS3},
	{SrcGlob: "/opt/weka/logs/s3*/*/*.stats*", DestDir: "weka/containers", Profile: ProfileS3},
	...
	{SrcGlob: "/opt/weka/logs/smbw*/*.stats*", DestDir: "weka/containers", Profile: ProfileSMBW},
	{SrcGlob: "/opt/weka/logs/smbw*/*/*.log*", DestDir: "weka/containers", Profile: ProfileSMBW},
...
func CollectLocal(tw *tar.Writer, archiveRoot, profile string, from, to time.Time, cmdTimeout time.Duration, nodeOnly bool, containerNames []string, extraCmds []CommandSpec) HostManifest {
	...
	for i, spec := range wekaToRun {
		...
		dest := filepath.Join(hostRoot, wekaSubdir, spec.Name+".txt")
		if err := addBytesToArchive(tw, dest, content); err != nil {
			warnf("[%s] could not add %s to archive: %v", hostname, spec.Name, err)
		}
	...
	activeContainerDirs := map[string]bool{}
	{
		...
		for _, c := range containers {
			...
			addLocalCmd(CommandSpec{
				...
				Cmd:       "weka local resources -C " + name,
				NodeLocal: true,
			}, ".txt")

			// net --stable: per-container network device details with gateway, PCI ID.
			addLocalCmd(CommandSpec{
				Name:      "weka_local_resources_net_" + name,
				Cmd:       "weka local resources -C " + name + " net --stable",
				NodeLocal: true,
			}, ".txt")

			// --stable: container resource config — detects auto-core allocation.
			addLocalCmd(CommandSpec{
...
func buildClusterWideCmds(profile string, from, to time.Time) []CommandSpec {
	all := append(append(append([]CommandSpec{}, defaultCommands...), buildWindowedDefaultCmds(from, to)...), buildProfileCommands(profile, from, to)...)
	var cmds []CommandSpec
...
func main() {
	...
	if *extraCommands {
		allBuiltin := append(append(append([]CommandSpec{}, defaultCommands...), buildWindowedDefaultCmds(from, to)...), buildProfileCommands(*profileStr, from, to)...)
	...
	if *localOnly {
		...
		writeArchive(outPath, toStdout, *profileStr, from, to, *cmdTimeout, *statsTimeout, *noStats, *nodeOnly, containerNames, nil, extraCmds, *compression)
		if !toStdout {
	...
	if len(clusterHosts) == 0 {
		if len(preDiscoveredHosts) > 0 {
			...
			if err != nil {
				...
				writeArchive(outPath, toStdout, *profileStr, from, to, *cmdTimeout, *statsTimeout, *noStats, false, containerNames, nil, extraCmds, *compression)
				if *upload && !toStdout {
	...
	if *upload && !toStdout {
		...
		go func() {
			...
			writeArchive(outPath, false, *profileStr, from, to, *cmdTimeout, *statsTimeout, *noStats, false, containerNames, nil, extraCmds, *compression)
	...
	writeMergedArchive(outPath, toStdout, results, *profileStr, from, to, *cmdTimeout, *statsTimeout, *noStats, collectionStart, extraCmds, *compression)
}
...
func writeArchive(outPath string, toStdout bool, profile string, from, to time.Time, cmdTimeout, statsTimeout time.Duration, noStats, nodeOnly bool, containerNames []string, extraManifests []HostManifest, extraCmds []CommandSpec, compression string) {
	clusterName := getClusterName()
	if globalAnonymizer.enabled {
		clusterName = "weka-cluster"
	}
	...
	if !nodeOnly {
		if manifest.WekaDown {
			...
			clusterTotal, clusterFailed = addClusterWideCmdsToArchive(tw, archiveRoot, profile, from, to, cmdTimeout, statsTimeout, noStats)
		}
		if len(extraCmds) > 0 {
...
func addClusterWideCmdsToArchive(tw *tar.Writer, archiveRoot, profile string, from, to time.Time, cmdTimeout, statsTimeout time.Duration, noStats bool) (totalCmds, failedCmds int) {
	clusterCmds := buildClusterWideCmds(profile, from, to)
	...
	if len(wekStatsCmds) > 0 {
		...
		logf("  [cluster] running %d weka stats commands (timeout: %s)", len(wekStatsCmds), statsTimeout)
		outs := runCommandsParallel(wekStatsCmds, statsTimeout, statsWorkers)
	...
	for i, spec := range clusterCmds {
		...
		dest := filepath.Join(archiveRoot, "cluster", wekaSubdir, spec.Name+".txt")
...
func writeMergedArchive(outPath string, toStdout bool, results []HostResult, profile string, from, to time.Time, cmdTimeout, statsTimeout time.Duration, noStats bool, collectionStart time.Time, extraCmds []CommandSpec, compression string) {
	clusterName := getClusterName()
	if globalAnonymizer.enabled {
		clusterName = "weka-cluster"
	}
	...
	addClusterWideCmdsToArchive(tw, archiveRoot, profile, from, to, cmdTimeout, statsTimeout, noStats)
